...
public class CodescannerHealthDescriptor extends AbstractHealthDescriptor {
     ...
     */
    public CodescannerHealthDescriptor(final HealthDescriptor healthDescriptor) {
    ...
    protected Localizable createDescription(final AnnotationProvider result) {
        if (result.getNumberOfAnnotations() == 0) {
            return Messages._Codescanner_ResultAction_HealthReportNoItem();
        }
        else if (result.getNumberOfAnnotations() == 1) {
            return Messages._Codescanner_ResultAction_HealthReportSingleItem();
        }
        else {
            return Messages._Codescanner_ResultAction_HealthReportMultipleItem(result.getNumberOfAnnotations());
        }
    }
}
