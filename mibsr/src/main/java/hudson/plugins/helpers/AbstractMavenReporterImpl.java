...
// -------------------------- OTHER METHODS --------------------------
     ...
     * {@inheritDoc}
     */
    public boolean preExecute( MavenBuildProxy build, MavenProject pom, MojoInfo mojo, BuildListener listener )
        throws InterruptedException, IOException
    {
...
// -------------------------- ENUMERATIONS --------------------------
    ...
    protected enum MojoExecutionReportingMode
    {
        ...
        ONLY_REPORT_ON_SUCCESS
            {
                Boolean isOkToContinue( MavenReporter reporter, MavenBuildProxy build, BuildListener listener,
                                        Throwable error )
                {
                                        ...
                                        Throwable error )
                {
                    if ( error != null )
                    {
                        listener.getLogger().println(
                            "[HUDSON] " + reporter.getDescriptor().getDisplayName() + " setting build to UNSTABLE" );
...
// -------------------------- OTHER METHODS --------------------------
         ...
         */
        abstract Boolean isOkToContinue( MavenReporter reporter, MavenBuildProxy build, BuildListener listener,
                                         Throwable error );
    }
}
